...
st.caption(
    "PRD: https://bit.ly/3gutmVr | Dataset: https://bit.ly/3D92jJ3 | Original code: https://bit.ly/3zfoieX | Article: https://bit.ly/3mulZkn"
)

with st.beta_expander("Show raw Kaggle dataset", expanded=False):
...
with a:
    st.write("✔️ Valid cases: ", len(valid))
...
st.caption("⏰ Execution time for feature selection: %.2f minutes" % (elapsed / 60))
...
if st.sidebar.checkbox("Show selected top features"):
    st.write(
        "Top %d features in order of importance are: %s"
...
if st.sidebar.checkbox("Run a credit card fraud detection model"):
    ...
    if classifier == "Logistic Regression":
        ...
        if imb_rect == "No Rectifier":
            compute_performance(
                model, X_train_sfs_scaled, y_train, X_test_sfs_scaled, y_test
            )
        elif imb_rect == "SMOTE":
            rect = smt
            ...
            compute_performance(
                model, X_train_bal, y_train_bal, X_test_sfs_scaled, y_test
            )
